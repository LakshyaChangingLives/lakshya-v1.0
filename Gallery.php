...
document.getElementById("albums").innerHTML+="<li><a href='#albums' onclick='getpics("+feed.data[i].id+",\""+feed.data[i].name+"\");' class='normaltip' title='"+feed.data[i].name+"'><img class='albums' src='http://graph.facebook.com/"+feed.data[i].cover_photo+"/picture' width='200' height='150'/></a></li>";}
}
}
...
for( var i=0;i<feed.photos.data.length;i++)
{

document.getElementById("albums").innerHTML+="<a href='"+feed.photos.data[i].source+"' rel='lightbox[plants]' title='my caption'><img src='"+feed.photos.data[i].images[0].source+"'class='gallerypics' width='150' height='100' alt='Plants: image 1 0f 10 thumbnails' /></a>" ;
} 
document.getElementById("albums").innerHTML+="</br><a href='#albums' onclick='gettoken();'><img src='images/close.png' float='right'/></a>";
}
...
function getpics(albumid,albumname){
